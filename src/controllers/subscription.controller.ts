...
// const SubscribeToChannel = asyncHandeler(async (req, res) => {
...
//   User.findByIdAndUpdate(req.user._id, { $inc: { subscriberCount: 1 } });

//   return res
//     .status(201)
//     .json(new ApiResponce(201, "Subscribed to channel successfully", subscription));
// });
...
const toggleSubscription = asyncHandeler(async (req, res) => {
    ...
    if (existingSubscription) {
        ...
        User.findByIdAndUpdate(req.user._id, { $inc: { subscriberCount: -1 } });
        return res.status(200).json(new ApiResponce(200, "Unsubscribed from channel successfully", {}));
    }
...
const getUserChannelSubscribers = asyncHandeler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponce(200, "Channel subscribers fetched successfully", subscribers));
});
...
const getSubscribedChannels = asyncHandeler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponce(200, "Subscribed channels fetched successfully", subscriptions));
})
